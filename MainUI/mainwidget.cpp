...
{
    //setWindowTitle(QStringLiteral("Desence Calculator V1.2 Release By DarinLi 20190123"));
    QString title = QStringLiteral("Desence Calculator ") + APPVersions +
    ...
    resize(d->m_mainWidgetUI->width()+5, d->m_mainWidgetUI->height() + titleBarHeight());

    titleBarCustomCreate();
    titleBarMenuSettingsCreate();

    int themevalue = d->m_mainWidgetUI->iniFilesValueRead(QStringLiteral("Base Information"),
...
void MainWidget::changeTheme(void)
{
    if(d->themeStytle1->isChecked())
    {
       ...
       d->m_mainWidgetUI->iniFilesSetValue(QStringLiteral("Base Information"),
